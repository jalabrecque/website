...
```{r}
library(readr)
ds <- suppressMessages(read_csv("/Users/jeremylabrecque/kDrive/Personal/rhl/start2026/keepers_updated.csv"))
...
library(knitr)
library(readr)
st <- suppressMessages(read_csv("/Users/jeremylabrecque/epidemiology/website/content/RHL/standings/Book1.csv"))
for (i in 2:9) {
  a <- st[,i]
  a[is.na(a)] <- 0
  st[,i] <- a
}
st$Total <- as.matrix(st[,2:9]) %*% as.matrix(c(8:1))
st <- st[order(st$Total,decreasing = TRUE),]

kable(st, row.names = F )
```
